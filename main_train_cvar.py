...
if __name__ == '__main__':
    ...
    Parameters
    """
    params = Hyperparameters(type_rm="CVaR", epsilon=1e-6, beta=0.2)
    repo_name = "agent-cvar0.2-eps1e-6"
    ...
    if preload:
        ...
        # evaluate Q, mu, F to have good approximations
        algo.initial_estimates(Nsims=params.algo_params["Nsims"],
                               Nepochs=params.algo_params["Nepochs_init"],
                               Nminibatch=params.algo_params["batch_F"])
        
    for epoch in range(params.algo_params["Nepochs"]):
        ...
        algo.update_Q(trajs=trajs,
                      Nepochs=params.algo_params["Nepochs_Q"],
                      Nminibatch=params.algo_params["batch_Q"])
        
        # estimate the first moment of costs-to-go
        algo.update_mu(trajs=trajs,
                       Nepochs=params.algo_params["Nepochs_mu"],
                       Nminibatch=params.algo_params["batch_mu"])
        
        # estimate the CDF of costs-to-go
        algo.update_F(trajs=trajs,
                      Nepochs=params.algo_params["Nepochs_F"],
                      Nminibatch=params.algo_params["batch_F"])
        
        # update the policy
        algo.update_pi(Nepochs=params.algo_params["Nepochs_pi"],
